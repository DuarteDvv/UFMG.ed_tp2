...
bool A_star(Adjacency_List &adj_list, double energy, int portals, double *heuristic)
{
    ...
    while (!pq.isEmpty())
    {
        ...
        for (int i = 0; i < adj_list.adj_list[u].getSize(); i++)
        {
            ...
            int v = adj_list.adj_list[u].getObject(i).getFirst();

            double weight = adj_list.adj_list[u].getObject(i).getSecond();
            ...
            if (dist[u] + weight < dist[v])
            {
                if (weight == 0)
                {
                    ...
                    pq.enqueue({{v, dist[v] + heuristic[v]}, used_portals + 1});
                }
                else
                { // c.c mantem a mesma quantidade
                    dist[v] = dist[u] + weight;
                    pq.enqueue({{v, dist[v] + heuristic[v]}, used_portals});
                }
            }
        }
    }
